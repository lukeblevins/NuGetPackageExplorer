...
namespace PackageExplorer.Framework.Xml
{
    /// <summary>
	...
	{
		private static readonly Lazy<ILogger> _logger = new Lazy<ILogger>(typeof(XmlConvert).LogError);
		...
		private static object PopulateTo(this XElement element, object instance)
		{
			...
			foreach (var attribute in element.Attributes())
			{
				...
				if (property != null)
				{
					...
					{
						_logger.Value.LogError($"Unknown property type: {property.PropertyType} <- @{attribute.Name.LocalName}");
					}
				}
				else
				{
					_logger.Value.LogError($"Unmapped property: @{attribute.Name.LocalName}");
				}
			}
			foreach (var child in element.Elements())
			{
				...
				if (property != null)
				{
					...
					{
						_logger.Value.LogError($"Unknown property type: {property.PropertyType} <- {child.Name.LocalName}");
					}
				}
				...
				{
					_logger.Value.LogError($"Unmapped property: {child.Name.LocalName}");
				}
			}
